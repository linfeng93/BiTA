...
class Template:
    ...
    def encode_multiturn(
        ...
        """
        system, history = self._format(query, resp, history, system)
        encoded_pairs = self._encode(tokenizer, system, history, template_tag=template_tag)
        ...
        for turn_idx, (query, resp) in enumerate(history):
            if turn_idx == 0: # llama2 template has no sep_ids
                if template_tag is not None:
                    if template_tag == "full":
                        query = self.prefix[0].replace("{{system}}", system) + query
                    elif template_tag == "short":
                        query = self.prefix[1] + query
                    else:
                        raise ValueError
                else:
                    query = self.prefix[0].replace("{{system}}", system) + query
...
class VicunaTemplate(Template):
    ...
    def _encode(
        ...
        encoded_pairs = []
        for turn_idx, (query, resp) in enumerate(history):
            if turn_idx == 0: # vicuna template has no sep_ids
                if template_tag is not None:
                    if template_tag == "full":
                        prompt = [self.prefix[0].replace("{{system}}", system) + " " + self.prompt[0]] 
                    elif template_tag == "short":
                        prompt = self.prompt
                    else:
                        raise ValueError
                ...
                prompt = self.prompt
            query_ids = self._convert_inputs_to_ids(tokenizer, context=prompt, query=query)
            resp_ids = self._convert_inputs_to_ids(tokenizer, context=[resp])
            encoded_pairs.append((bos_ids + query_ids, resp_ids + eos_ids))
        return encoded_pairs
...
def register_template(
    ...
    if "llama2" in name:
        ...
        template_class = VicunaTemplate
    elif "falcon" in name:
        template_class = FalconTemplate
    else:
...
register_template(
    name="llama2",
    # prefix[0] for default template, prefix[1] for short template
    prefix=[
        "<<SYS>>\n{{system}}\n<</SYS>>\n\n",
        ""
    ],
...
register_template(
    ...
    prompt=[
        "USER: {{query}} ASSISTANT:"  # 冒号后面如果增加一个空格的话，vicuna 容易生成中文回答
    ],
    system=(
        "A chat between a curious user and an artificial intelligence assistant. "
        "The assistant gives helpful, detailed, and polite answers to the user's questions."
...
r"""
Supports: https://github.com/hiyouga/LLaMA-Factory
"""
register_template(
    name="falcon",
    prefix=[
