...
def init_adapter(
    ...
    Support full-parameter, P-Tuning, freeze and LoRA training.
    ...
    if finetuning_args.finetuning_type in ("pt", "pt2"):
        logger.info("Fine-tuning method: PromptTuning")
        trainable_layers = ["mask_tokens", "prefix_encoder"]
        for name, param in model.named_parameters():
            if not any(trainable_layer in name for trainable_layer in trainable_layers):
                param.requires_grad_(False)
            else:
                param.data = param.data.to(torch.float32)    
        logger.info("Trainable layers: " + ", ".join(trainable_layers))
