...
from modeling_llama_if import LlamaForCausalLM
from modeling_falcon_if import FalconForCausalLM
...
from llmtuner.hparams import FinetuningArguments, DataArguments
...
def load_model_and_tokenizer(
    ...
    setattr(config, "finetuning_type", finetuning_args.finetuning_type)
    if data_args is not None:
        setattr(config, "mask_num", data_args.mask_num)
        setattr(config, "mask_diff", data_args.mask_diff)
        setattr(config, "max_efficient_groups", data_args.max_efficient_groups)
    if finetuning_args.finetuning_type == "pt2":
        setattr(config, "prompt_num", data_args.prompt_num)
        setattr(config, "prefix_dropout_prob", model_args.prefix_dropout_prob)
        setattr(config, "prefix_projection", model_args.prefix_projection)
        setattr(config, "prefix_hidden_size", model_args.prefix_hidden_size)
    ...
    if 'llama' in model_name:  # support efficient training for llama
        ...
        ## vicuna use the llama structure
        model = LlamaForCausalLM.from_pretrained(
            model_to_load,
            config=config,
            torch_dtype=model_args.compute_dtype,
